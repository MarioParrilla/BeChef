...
class EditProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String oldUsername = dataProfileProvider!.username.isEmpty
        ? 'username'
        : dataProfileProvider!.username;
    String oldDescription = dataProfileProvider!.description.isEmpty
        ? 'description'
        : dataProfileProvider!.description;
    String oldUrlImg = dataProfileProvider!.urlImgTemp.isEmpty
        ? 'https://d500.epimg.net/cincodias/imagenes/2016/07/04/lifestyle/1467646262_522853_1467646344_noticia_normal.jpg'
        : dataProfileProvider!.urlImgTemp;

    Future<void> modifyUser() async {
      if (dataProfileProvider!.usernameChanged ||
          dataProfileProvider!.descriptionChanged ||
          dataProfileProvider!.urlImgChanged) {
        dynamic newUser = await dataUserLoggedService.changeBasicDataUser(
            context,
            newUsername == '' ? oldUsername : newUsername,
            newDescription == '' ? oldDescription : newDescription,
            newImg);

        if (newUser.runtimeType == User) {
          if (newImg == null) {
            dataProfileProvider!.username =
                newUser.username == '' ? oldUsername : newUser.username;
      ...
      child: Form(
        ...
        children: [
          CustomInputField(
              ...
              onChange: (String value) => usrChanged(value)),
          CustomInputField(
              color: Colors.deepOrange,
              minLines: 2,
              maxLines: 3,
